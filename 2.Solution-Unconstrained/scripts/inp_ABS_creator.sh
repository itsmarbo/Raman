...
for file in *.com; do
    ...
    touch "${filename}.ABS.inp"

    # Write the input parameters to the .inp file
    cat << EOF > "${filename}.ABS.inp"
\$\$\$
...
FORCE_REAL   =   YES
;INPUT DATA FILES
STATE1_FILE  =   /home/marlene/uracil/1.S0_snapshots/0-100/${filename2}.S0.fcc
...
EOF
    ...
    echo "Input created ${filename}.ABS.inp"
done
